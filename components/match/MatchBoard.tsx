    ...
    <>
      {spots.map((s, i) => (
        ...
        >
          ...
          <span
            className="mono"
            style={{ marginTop: 3, fontSize: 8.5, lineHeight: 1, color: "var(--ink)", whiteSpace: "nowrap", textShadow: "0 1px 3px rgba(0,0,0,0.9)" }}
          >
            {s.name}
          </span>
        </div>
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-2xl" style={{ background: "linear-gradient(135deg, #08130c 0%, #060d09 100%)" }}>
      ...
      <svg viewBox="0 0 100 56" preserveAspectRatio="none" className="absolute inset-0 w-full h-full" style={{ opacity: 0.4 }}>
        <rect x="1" y="1" width="98" height="54" fill="none" stroke="var(--green-line)" strokeWidth="0.3" />
        <line x1="50" y1="1" x2="50" y2="55" stroke="var(--green-line)" strokeWidth="0.3" />
        <circle cx="50" cy="28" r="7" fill="none" stroke="var(--green-line)" strokeWidth="0.3" />
        <rect x="1" y="17" width="12" height="22" fill="none" stroke="var(--green-line)" strokeWidth="0.3" />
        <rect x="87" y="17" width="12" height="22" fill="none" stroke="var(--green-line)" strokeWidth="0.3" />
      </svg>

      {/* formation labels */}
      <span className="mono absolute top-3 left-4" style={{ fontSize: 10, color: "var(--green-bright)", letterSpacing: "0.1em" }}>
